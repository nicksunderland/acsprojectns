...
foo <- readr:: read_csv("/Users/nicholassunderland/Documents/2.Medical_work/5.Bristol/acsprojectns/acsprojectns/materials/dummy_databases/swd_measurement_dummy_database.csv")

# Write out the table into the MySQL database
dbWriteTable(db_connection,
             value = foo,
             row.names = FALSE,
             name = "swd_measurement",
             overwrite = TRUE)
