...
index_acs_diagnoses <- msrv$sus$apc_episodes |>
  # Only cohort NHS numbers
  dplyr::filter(nhs_number %in% cohort_ids) |>
  # Time window
  dplyr::filter(episode_start > acs_date_min & episode_end < acs_date_max) |>
  # collect to local
  icdb::run() |>
  ...
  # make sure that the episode overlaps the spell 
  dplyr::filter(lubridate::int_overlaps(int1 = lubridate::interval(episode_start, episode_end),
...
index_acs_procedures <- index_acs |>
