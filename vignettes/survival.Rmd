...
#TODO ?need this bit
  ...
  # dplyr::slice_min(tstop, with_ties=F)
...
km_plot_lipids <- ggsurvplot(
           fit          = surv_fit,
           #facet.by = c("fudge_cambridge_segment"),
           surv.scale   = "percent",
           xscale       = 365.25,
           break.time.by= 365.25,
           conf.int      = TRUE,
           ...
           legend       = c(0.7,0.3),
           legend.title = "",
           legend.labs = levels(surf_data$lipid_strat),
           #risk.table = TRUE,
           ggtheme      = theme_classic())
km_plot_lipids
...
outcome_groupings = list("emergency ACS readmission" = "^acs_[a-z]+_*[a-z_]*emergency$",
                         "emergency angiogram+/-PCI" = c("angiogram_emergency","pci_emergency")) #"(?<!presence_of_)cabg_emergency")
# check which outcomes you're about to look at (what have the regexes above found?)
unique(all_subsequent_events$event_desc)[grepl(paste0(unlist(outcome_groupings),collapse="|"),unique(all_subsequent_events$event_desc),perl=TRUE)]
# create data
surf_data <- acsprojectns::create_survival_timings(all_subsequent_events, outcome_groupings) |> 
  dplyr::left_join(dat_demographics |> dplyr::select(nhs_number, age, fudge_cambridge_segment), by="nhs_number") |>
  dplyr::left_join(index_acs_main |> dplyr::select(nhs_number, primary_diagnosis, procedure), by="nhs_number") |>
  dplyr::mutate(strategy = factor(grepl("angiogram|pci|cabg", procedure), 
                                  levels = c(F,T),
                                  labels = c("Conservative initial strategy", "Invasive initial strategy")), 
  ...
  dplyr::group_by(nhs_number, outcome) |> 
  dplyr::slice_min(tstop, with_ties=F) 
#  dplyr::filter(grepl("acs_nstemi", primary_diagnosis))
#  dplyr::filter(!acs_stemi, !acs_nstemi, !acs_unstable_angina)

# Create the Survival object - for any readmission
surv_tt_obj <- survival::Surv(surf_data$tstart, surf_data$tstop, surf_data$status)
# Survival fit
surv_fit <- survfit(surv_tt_obj ~ outcome + strategy, data=surf_data)
# Survival table
survtbl <- summary(surv_fit)
# Plot 
km_plot <- ggsurvplot(
             ...
             conf.int      = TRUE,
             conf.int.alpha=0.25,
             palette      = wes_palette("GrandBudapest1", length(levels(surf_data$strategy)), type="discrete"), #c("darkgreen", "darkred"),
             censor.shape ="",
             ...
             legend       = c(0.8,0.15),
             legend.title = "",
             legend.labs =  levels(surf_data$strategy),
             # risk.table = TRUE, 
             ggtheme      = theme_classic())
km_plot
