...
subsequent_measurements <- msrv$swd$swd_measurement |>
  ...
  #dplyr::filter(event_start >= index_start) |>
  # extract procedure grouping / coding
  dplyr::mutate(event_desc = paste(measurement_name, measurement_group)) |>
...
```
## Survival - cardiovascular death
## Table - cause of death (in first 12 months)
```{r cv_death, results="markup"}
# outcome_groupings = list("any_death" = "death")
# 
#   # keep only deaths and emergency admissions
#   # dplyr::filter(grepl("emergency$|^death", event_desc)) |>
...
# Survival fit
surv_fit <- survfit(surv_tt_obj ~ outcome + fudge_cambridge_segment, data=lipids)
# Survival table
survtbl <- summary(surv_fit)
# survival at time points
time_points <- seq(1, 365*2, 90)
data.frame("time"=time_points, "survival"=round(survtbl$ surv[time_points], digits=3))
...
post_discharge_blanking = 120 #days

#lipid strategy
lipid_strat <- all_subsequent_events |>
  # only take statin or inclisiran prescription events
  dplyr::filter(grepl("statin", event_desc)) |>
  # only take prescription events within the blanking period
  dplyr::filter(event_start < (index_end + lubridate::duration(post_discharge_blanking, units="days"))) |>
  # order by the most potent strategy and take that for each patient
  dplyr::mutate(event_desc = factor(event_desc, levels = c("high_statin", "subopt_statin"))) |>
  dplyr::group_by(nhs_number) |>
...
surf_data <- acsprojectns::create_survival_timings(all_subsequent_events, outcome_groupings) |> 
  ...
  dplyr::mutate(lipid_strat = as.character(lipid_strat),
                ...
                lipid_strat = dplyr::if_else(tstop<post_discharge_blanking, "blanking_window", lipid_strat), 
...
## Survival (cardiovascular death) by lipid prescribing at 4 months post discharge
```{r cv_survival_by_lipid_prescribing, results="markup"}
# look at the unique outcome options and choose those to assess; filter some stuff out to make it easier
#unique(all_subsequent_events$event_desc[!grepl("elective|emergency|[A-Z]\\d+", all_subsequent_events$event_desc)])
# 
# #survival data
# outcome_groupings = list("acs" = c("acs_[a-z]+"))
# 
# surf_data <- acsprojectns::create_survival_timings(all_subsequent_events, outcome_groupings) |> 
#   dplyr::left_join(dat_demographics |> dplyr::select(nhs_number, fudge_cambridge_segment), by="nhs_number") |>
#   dplyr::left_join(lipid_strat, by="nhs_number") |>
...
#                 lipid_strat = factor(lipid_strat, levels=c("high_statin", "subopt_statin","none","blanking_window"))) |>
#   dplyr::group_by(nhs_number) |>
#   dplyr::mutate(had_blanking_event = any(lipid_strat=="blanking_window")) |>
#   dplyr::filter(!had_blanking_event)
#                 
# # Create the Survival object - for any readmission
# surv_tt_obj <- survival::Surv(surf_data$tstart, surf_data$tstop, surf_data$status)
# # Survival fit
...
#            conf.int      = TRUE,
#            conf.int.alpha=0.25,
#            palette      = c("darkgreen", "orange3", "darkred"),
#            censor.shape ="",
#            xlab = "Time from index ACS event (years)",
#            ylab = "Survival",
#            ylim = c(0.0, 1.0),
...
#            legend       = c(0.7,0.3),
#            legend.title = "",
#            legend.labs = c("High dose statin", "Sub-optimal dose statin", "No statin"),
#            risk.table = TRUE,
#            ggtheme      = theme_classic())
# km_plot_lipids
