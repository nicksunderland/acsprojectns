...
outcome_groupings = list("acs_readmission" = c("^acs_[a-z]+_[a-z]*_emergency$"),
                         "emergency_angio" = c("angiogram_emergency", "pci_emergency"))
                                              #TODO add CABG
surf_data <- acsprojectns::create_survival_timings(all_subsequent_events, outcome_groupings) |> 
  dplyr::left_join(dat_demographics |> dplyr::select(nhs_number, fudge_cambridge_segment), by="nhs_number") |>
  dplyr::left_join(index_acs_main |> dplyr::select(nhs_number, any_angiogram), by="nhs_number") |>
  dplyr::mutate(any_angiogram = factor(dplyr::if_else(!is.na(any_angiogram),T,F),
                                       levels = c(T,F),
                                       labels = c("Index angiogram", "No index angiogram")))
...
# Plot 
  km_plot <- ggsurvplot(
             fit          = surv_fit,
             facet.by     = c("fudge_cambridge_segment", "outcome"),
             surv.scale   = "percent",
             xscale       = 365.25,
             break.time.by= 365.25,
             conf.int      = TRUE,
             ...
             # paste(rep(levels(surf_data$outcome), each=length(levels(surf_data$any_angiogram))),
             #                      levels(surf_data$any_angiogram), 
             #                      sep=" - "),
             #TODO when faceting try removing the risk table
             # risk.table = TRUE, 
             ggtheme      = theme_classic())
  km_plot
